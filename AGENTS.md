...
| | 公告 MCP | market-data MCP | market-data-chart MCP |
|---|---|---|---|
| 定位 | 公告/提交文档（非结构化）的发现、定位、阅读 | 行情/财报/指标/概况（结构化）数据获取 | 行情绘图（K线/折线，事件标注/高低点） |
| 数据源 | A/B/H 股=东方财富；美股=SEC EDGAR；LOCAL-=本地材料 | 行情=新浪单源；财报=东财 datacenter；股本/美股后复权=iFinD | 复用 market-data get_quote（不复制取数代码） |
| 状态 | **已实现、完善**（不要动坏） | **行情 + 财报三表已实现**；财务比率/公司概况待实现 | **已实现**（get_quote_chart，2026-08 改名 market-data-chart + 事件/高低点） |
...
3.5 **所有工具不做公司级专门适配**（市场公司太多，逐家适配不可维护）：提取器/解析器只按**站点级或市场级规则**写（如 AlphaSpread/AlphaStreet 各一个提取器），不按某家公司页面结构做矫正；页面内容差异（开场白措辞、参与者列表、speaker 格式等）交给 AI 读正文自行判断，脚本不替 AI 做内容判断。
...
| **docs/下游反馈-get_quote未完成日线-2026-08-17.md** | 下游简报踩中的未完成日线现象、复测与未决方向（待拍板） | 已拍板决策（拍板后写入架构设计） |
