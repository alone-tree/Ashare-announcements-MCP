...
- 批处理 JSON CLI：供确定性脚本批量查询、检查、检索和读取公告，与 MCP 共用缓存。
...
支持 `query_batch`、`inspect_batch`、`search_batch` 和 `read_batch`。默认检索和读取不启用 OCR；默认读取上限为 20,000 字符。公告类型、长度和扫描比例等业务分流由调用方决定。
