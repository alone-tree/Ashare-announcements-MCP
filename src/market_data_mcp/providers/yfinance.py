...
口径注意：yfinance Ordinary Shares 扣库存股（0700.HK 与 iFinD total_shares 差 87M），
缓存带 source 元数据，不跨源填充。
"""
...
def fetch_shares(
    ...
    """请求 yfinance 股本（total_shares，逐日）并写缓存。
    仅港美股。返回 {ok, source, path, new_items, date_range, error, notes}。"""
    if mc.market not in ("HK", "US"):
        return {"ok": False, "source": SOURCE, "path": None, "new_items": 0,
                "date_range": None, "error": f"yfinance 仅支持港美股（收到 {mc.market}）", "notes": None}
    ...
    try:
        ...
        audit.log_request(root, source=SOURCE, market=mc.market, code=code,
                          api="Ticker.get_shares_full", fields="total_shares",
                          start=start, end=end, ok=False, elapsed=elapsed, error=str(exc))
        return {"ok": False, "source": SOURCE, "path": None, "new_items": 0,
                "date_range": None, "error": f"yfinance 股本请求失败：{exc}", "notes": None}
    finally:
        for k in _PROXY_KEYS:
            os.environ.pop(k, None)
    ...
    if df is not None and len(df):
        for ts, v in df.items():
            if v is None:
                continue
            fresh.append({"date": ts.strftime("%Y-%m-%d"), "total_shares": float(v)})
    ...
    if not fresh:
        ...
        return {"ok": True, "source": SOURCE, "path": None, "new_items": 0,
                "date_range": None, "error": None, "notes": None}
    ...
    if existing and existing.get("meta", {}).get("source") != SOURCE:
        ...
        audit.log_request(root, source=SOURCE, market=mc.market, code=code,
                          api="Ticker.get_shares_full", fields="total_shares",
                          start=start, end=end, ok=True, elapsed=elapsed)
        return {"ok": True, "source": SOURCE, "path": None, "new_items": len(fresh),
    ...
    return {"ok": True, "source": SOURCE, "path": path, "new_items": len(fresh),
            "date_range": date_range, "error": None, "notes": None}
